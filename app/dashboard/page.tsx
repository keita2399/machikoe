...
export default async function DashboardPage() {
  const topics = await getDashboardData();
            ...
            return (
              <div key={topic.id} className="surface-card">
                      ...
                      {(["agree", "conditional", "disagree"] as const).map((s) => {
                        ...
                        return (
                          <div key={s} style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                            <span style={{
                              fontSize: "12px", fontWeight: 500, width: "80px", flexShrink: 0,
                              color: m.color,
                            }}>
                              {m.label}
                            </span>
                ...
                <div style={{ marginTop: "14px", textAlign: "right" }}>
                  <Link href={`/topics/${topic.id}`} className="btn-mc-primary" style={{ fontSize: "12.5px", padding: "7px 12px" }}>
                    この議題に意見する →
                  </Link>
                </div>
              </div>
