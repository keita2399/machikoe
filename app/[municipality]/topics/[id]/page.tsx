  ...
  if (!topic || !mc) {
    return (
      <div style={{ maxWidth: "40rem", margin: "0 auto", padding: "4rem 1rem", textAlign: "center" }}>
        ...
        <Link href={`/${municipality}`} style={{ color: "var(--mc-primary-700)", fontSize: "14px" }}>← 一覧に戻る</Link>
      </div>
    );
  }
  ...
  return (
    <div style={{ maxWidth: "40rem", margin: "0 auto", padding: "0 1rem 4rem" }}>
      ...
      <div style={{ padding: "12px 0 8px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <Link href={`/${municipality}`} className="icon-btn" aria-label="戻る" style={{ border: "none" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }}>
          {(["resident", "business"] as UserType[]).map((type) => {
            ...
            const labels: Record<string, { title: string; sub: string }> = {
              resident: { title: "住民として", sub: `${mc.name}に住んでいる立場` },
              business: { title: "事業者として", sub: `${mc.name}で商売している立場` },
            };
            return (
              <button
                key={type}
      ...
      {userType && !loadingImpact && impact && (
        <div>
          <button
            onClick={() => router.push(`/${municipality}/topics/${id}/opinion?userType=${userType}`)}
