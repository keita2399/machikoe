...
export default function OpinionPage() {
  const { municipality, id } = useParams<{ municipality: string; id: string }>();
  ...
  function handleXShareToTown() {
    if (!mc) return;
    const text = encodeURIComponent(`${mc.xHandle} ${editedDraft}\n\n${mc.xHashtags}`);
  ...
  async function handleCopy() {
    ...
    setCopied(true);
    window.open(mc.inquiryUrl, "_blank");
  }
    ...
    return (
      <div style={{ maxWidth: "40rem", margin: "0 auto", padding: "4rem 1rem", textAlign: "center" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "8px", marginTop: "2rem" }}>
          <Link href={`/${municipality}/dashboard`} className="btn-mc-primary" style={{ justifyContent: "center", padding: "12px 20px" }}>
            みんなの声を見る →
          </Link>
          <Link href={`/${municipality}`} style={{ display: "block", textAlign: "center", fontSize: "13.5px", color: "var(--mc-primary-700)" }}>
            ← 議題一覧に戻る
          </Link>
        </div>
      </div>
  ...
  return (
    <div style={{ maxWidth: "40rem", margin: "0 auto", padding: "0 1rem 4rem" }}>
      ...
      <div style={{ padding: "12px 0 8px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <Link href={`/${municipality}/topics/${id}`} className="icon-btn" aria-label="戻る" style={{ border: "none" }}>
        ...
        <div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
            ...
            <button onClick={handleXShareToTown} className="btn-mc-ghost" style={{ width: "100%", justifyContent: "space-between" }}>
              ...
              <span style={{ color: "var(--mc-ink-400)", fontSize: "12px" }}>{mc.xHandle} に送る</span>
            </button>
